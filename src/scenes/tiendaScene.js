...
class tiendaScene extends Phaser.Scene{
    ...
    create(){
        ...
        this.spriteMapas = this.add.sprite(gameConfig.scale.width / 3,gameConfig.scale.height/3,'botonMapas').setScale(0.6);
        this.spriteMapas.setInteractive().on('pointerdown', () => this.scene.start("TiendaMapasScene"));

        this.spritePersonajes = this.add.sprite((gameConfig.scale.width / 3)*2,gameConfig.scale.height/3,'botonPersonajes').setScale(0.6);
        this.spritePersonajes.setInteractive().on('pointerdown', () => this.scene.start("TiendaPersonajesScene"));

        this.spriteArmas = this.add.sprite(gameConfig.scale.width / 3,(gameConfig.scale.height/3)*2,'botonArmas').setScale(0.6);
        this.spriteArmas.setInteractive().on('pointerdown', () => this.scene.start("TiendaArmasScene"));

        this.spritePaquetes = this.add.sprite((gameConfig.scale.width / 3)*2,(gameConfig.scale.height/3)*2,'botonPaquetes').setScale(0.6);
        this.spritePaquetes.setInteractive().on('pointerdown', () => this.scene.start("TiendaPaquetesScene"));

        this.spriteSalir = this.add.sprite(gameConfig.scale.width / 8,(gameConfig.scale.height/8)*6.5,'botonSalirTienda').setScale(0.6);
        this.spriteSalir.setInteractive().on('pointerdown', () => this.scene.start("MenuPrincipalScene"));

        this.spriteRecargar = this.add.sprite((gameConfig.scale.width / 8)*7.8,(gameConfig.scale.height/8)*6.5,'botonRecargar').setScale(0.6);
